...
Una vez que lo hayamos solucionado les avisaremos para que hagan un `pull` a su repositorio local con los cambios que hayamos efectuado; mientras tanto les recomendamos correr los tests de manera local con `pytest` para obtener feedback outomático. Mil disculpas, gracias por tanto.
...
5. Utiliza el algoritmo DBSCAN (Density-Based Spatial Clustering of Applications with Noise) de sklearn para agrupar en clusters los datos (en el notebook `ej1/ej1_explore_train.ipynb`).
